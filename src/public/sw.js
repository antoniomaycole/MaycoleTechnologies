...
const CACHE_FIRST = ['.js', '.css', '.png', '.jpg', '.jpeg', '.svg', '.webp', '.woff', '.woff2'];
...
async function handleFetchRequest(request, url) {
  try {
    ...
    if (request.destination === 'document') {
      ...
      return (
        offlineResponse ||
        new Response('Offline - MaycoleTracker™ unavailable', {
          status: 503,
...
self.addEventListener('push', (event) => {
  ...
  event.waitUntil(self.registration.showNotification('MaycoleTracker™', options));
});
...
self.addEventListener('periodicsync', (event) => {
  ...
  if (event.tag === 'inventory-refresh') {
    event.waitUntil(refreshInventoryData());
  }
});
